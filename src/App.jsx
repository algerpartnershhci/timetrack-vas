...
import { LogIn, LogOut, Shield, Download, Edit2, Trash2, ArrowLeft, Eye, EyeOff, AlertCircle, User, CalendarDays, Timer, Pause, Play, Zap, AlertTriangle, UserPlus, Users, X, Loader2 } from "lucide-react";
...
function Btn({ onClick, children, variant="primary", disabled, fullWidth, size="md", type }) {
  ...
  return <button type={type} style={{ ...base, ...sz, ...vars[variant] }} onClick={disabled ? undefined : onClick}>{children}</button>;
}
...
function EmployeeScreen({ employees, activeSessions, onClockIn, onStartOT, onPause, onResume, onClockOut, onBack }) {
  ...
  useEffect(() => {
    if (!selected) { setTodayEntries([]); return; }
    const today = todayLA();
    const q = query(collection(db, "entries"), where("employee", "==", selected), where("date", "==", today));
    return onSnapshot(q, snap => setTodayEntries(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
  }, [selected]);

  // Auto-pause regular sessions at 8h (hard cap)
  useEffect(() => {
    if (!selected) return;
    const s = activeSessions[selected];
    if (!s) return;
    if (s.type === "regular" && s.status === "working" && workedMs(s) >= DAILY_LIMIT_MS) {
      onPause(selected);
    }
  }, [tick, selected, activeSessions]);
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--navy)", padding:20, maxWidth:480, margin:"0 auto" }}>
      ...
      <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:14, padding:20, marginBottom:16 }}>
        ...
        {employees.length === 0 ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {employees.map(emp => {
              ...
              return (
                <button key={emp.id} onClick={() => setSelected(emp.name)} style={{ background: selected === emp.name ? "var(--accent)" : "var(--navy)", border: `1.5px solid ${selected === emp.name ? "var(--accent)" : "var(--border)"}`, borderRadius:10, padding:"12px 16px", cursor:"pointer", textAlign:"left", color:"var(--text)", fontFamily:"'DM Sans'", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <span style={{ fontWeight:600, fontSize:14 }}>{emp.name}</span>
                  <span style={{ fontSize:10, fontWeight:700, padding:"3px 9px", borderRadius:20, background:bg, color: selected === emp.name ? "#fff" : fg }}>{badge}</span>
                </button>
              );
            })}
          </div>
      ...
      {selected && (
        <div style={{ background:"var(--card)", border: session?.type === "overtime" ? "1.5px solid var(--purple)" : "1.5px solid var(--border)", borderRadius:14, padding:20 }}>
          <div style={{ fontWeight:700, fontSize:16, marginBottom:4 }}>{selected}</div>
    ...
    <>
      {atLimit && (
        <div style={{ background:"rgba(247,107,107,.1)", border:"1px solid rgba(247,107,107,.4)", borderRadius:10, padding:14, marginTop:14, display:"flex", gap:12, alignItems:"flex-start" }}>
          ...
          <div>
            ...
            <div style={{ color:"var(--muted)", fontSize:12, lineHeight:1.5 }}>Please clock out. If you need to do overtime, ask admin to authorize OT after you clock out.</div>
          </div>
        </div>
      )}
      <div style={{ background:"var(--navy)", borderRadius:10, padding:18, marginTop:14, textAlign:"center" }}>
        ...
        <div style={{ fontSize:12, color:"var(--muted)", marginTop:6 }}>
          ...
          {(session.totalPausedMs > 0 || (session.status === "paused" && session.pausedAt)) && <> • Break total: {msToHMS((session.totalPausedMs || 0) + (session.status === "paused" && session.pausedAt ? Date.now() - session.pausedAt : 0))}</>}
        </div>
...
function EditModal({ entry, onSave, onClose }) {
  ...
  const save = () => {
    ...
    let workedMillis = outTs - inTs - (entry.breakMs || 0);
    if (entry.type === "regular" && workedMillis > DAILY_LIMIT_MS) workedMillis = DAILY_LIMIT_MS;
    onSave({ ...entry, date: dateStr, clockIn: inTs, clockOut: outTs, duration: msToHMS(workedMillis) });
  };
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:20 }}>
      <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:16, padding:28, width:"100%", maxWidth:400 }}>
        ...
        {entry.type === "regular" && <p style={{ color:"var(--muted)", fontSize:11, marginTop:12 }}>Note: Regular entries are capped at 8 hours.</p>}
...
// ║ EMPLOYEE MANAGEMENT
// ╚════════════════════════════════════════════════════════════════════════
function EmployeeManagement({ employees, onAdd, onRemove }) {
  ...
  const add = () => {
    ...
    if (!name) return;
    if (employees.some(e => e.name.toLowerCase() === name.toLowerCase())) return alert("Employee with this name already exists");
    onAdd(name);
    setNewName("");
  };
    ...
    <>
      ...
      <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:12, padding:16, marginBottom:16 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
          <span style={{ fontSize:12, fontWeight:700, color:"var(--muted)", textTransform:"uppercase", letterSpacing:.8, display:"flex", alignItems:"center", gap:8 }}><Users size={14} /> Manage Employees ({employees.length})</span>
        </div>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:14 }}>
          <input
            ...
            placeholder="Add new employee (full name)"
            style={{ flex:1, background:"var(--navy)", border:"1.5px solid var(--border)", borderRadius:8, color:"var(--text)", padding:"10px 14px", fontSize:14, fontFamily:"'DM Sans'", outline:"none" }}
          />
          <Btn variant="primary" size="sm" onClick={add} disabled={!newName.trim()}><UserPlus size={14} /> Add</Btn>
        </div>
        ...
        {employees.length === 0 ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6, maxHeight:240, overflowY:"auto" }}>
            {employees.map(emp => (
              <div key={emp.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"var(--navy)", borderRadius:8, padding:"10px 14px" }}>
            ...
            <tr key={e.id} style={{ borderBottom: i < entries.length-1 ? "1px solid var(--border)" : "none" }}>
              ...
              <td style={{ padding:"12px 14px" }}>
                <div style={{ display:"flex", gap:6 }}>
                  <button onClick={() => onEdit(e)} title="Edit" style={{ background:"rgba(79,142,247,.15)", border:"none", borderRadius:6, padding:"6px 8px", cursor:"pointer", color:"var(--accent)" }}><Edit2 size={13} /></button>
                  <button onClick={() => onDelete(e.id)} title="Delete" style={{ background:"rgba(247,107,107,.15)", border:"none", borderRadius:6, padding:"6px 8px", cursor:"pointer", color:"var(--red)" }}><Trash2 size={13} /></button>
                </div>
              </td>
            </tr>
          ))}
...
// ║ ADMIN DASHBOARD
// ╚════════════════════════════════════════════════════════════════════════
function AdminDashboard({ employees, activeSessions, onSaveEntry, onDeleteEntry, onToggleEntryOT, onAddEmployee, onRemoveEmployee, onBack }) {
  ...
  // Subscribe to entries based on date filter
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      const list = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => b.clockIn - a.clockIn);
      setEntries(list);
  ...
  const sumDuration = (list) => list.reduce((acc, e) => acc + (e.clockOut - e.clockIn - (e.breakMs || 0)), 0);
  const regularMs = sumDuration(regularFiltered);
  const otMs      = sumDuration(otFiltered);
  ...
  const handleExport = () => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Time Report</title>
<style>body{font-family:'Segoe UI',Arial,sans-serif;padding:40px;color:#111;background:#fff}h1{font-size:22px;font-weight:700;margin-bottom:4px;color:#0f1117}h2{font-size:16px;font-weight:700;margin-top:32px;margin-bottom:10px;color:#0f1117;padding-bottom:6px;border-bottom:2px solid #0f1117}h2.ot{color:#7c3aed;border-bottom-color:#7c3aed}.meta{color:#555;font-size:13px;margin-bottom:6px}.badge{display:inline-block;background:#0f1117;color:#fff;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600;margin-bottom:18px;margin-right:6px}table{width:100%;border-collapse:collapse;font-size:13px;margin-bottom:8px}th{background:#0f1117;color:#fff;padding:10px 12px;text-align:left;font-weight:600}th.ot{background:#7c3aed}td{padding:9px 12px;border-bottom:1px solid #eee}tr:nth-child(even) td{background:#f8f8f8}.summary{margin-top:8px;font-size:13px;color:#444;display:flex;gap:24px;flex-wrap:wrap}.summary span{font-weight:700;color:#0f1117}.empty{padding:20px;color:#999;font-size:13px;font-style:italic}.footer{margin-top:24px;font-size:11px;color:#999;border-top:1px solid #eee;padding-top:12px}@media print{body{padding:20px}}</style>
</head><body>
<h1>Employee Time Report</h1>
<div class="meta">Period: <strong>${periodLabel}</strong> &nbsp;|&nbsp; Employee: <strong>${empLabel}</strong> &nbsp;|&nbsp; Timezone: <strong>${tzAbbr(Date.now())} (Los Angeles)</strong></div>
<div class="badge">Generated: ${new Date().toLocaleString("en-US", { timeZone: TZ })}</div>
<h2>Regular Time Entries</h2>
${regularFiltered.length > 0 ? `<table><thead><tr><th>Employee</th><th>Date</th><th>Clock In</th><th>Clock Out</th><th>Break</th><th>Total</th></tr></thead><tbody>${regularRows}</tbody></table><div class="summary"><div>Entries: <span>${regularFiltered.length}</span></div><div>Total Hours: <span>${fmtTotal(regularMs)}</span></div></div>` : '<div class="empty">No regular entries in this period.</div>'}
<h2 class="ot">⚡ Overtime Entries</h2>
${otFiltered.length > 0 ? `<table><thead><tr><th class="ot">Employee</th><th class="ot">Date</th><th class="ot">Clock In</th><th class="ot">Clock Out</th><th class="ot">Break</th><th class="ot">Total</th></tr></thead><tbody>${otRows}</tbody></table><div class="summary"><div>OT Entries: <span>${otFiltered.length}</span></div><div>OT Hours: <span style="color:#7c3aed">${fmtTotal(otMs)}</span></div></div>` : '<div class="empty">No overtime entries in this period.</div>'}
<div class="footer">All times in PST/PDT (Los Angeles, California) • Combined total: ${fmtTotal(regularMs + otMs)}</div>
</body></html>`;
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--navy)", padding:20 }}>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        ...
        <EmployeeManagement employees={employees} onAdd={n => { onAddEmployee(n); showToast(`${n} added`); }} onRemove={id => { onRemoveEmployee(id); showToast("Employee removed", "warn"); }} />

        <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:12, marginBottom:20 }}>
          {[
            ["Regular Entries", regularFiltered.length, "var(--accent)"],
            ["Regular Hours",   fmtTotal(regularMs),    "var(--green)"],
            ["OT Entries",      otFiltered.length,      "var(--purple)"],
            ["OT Hours",        fmtTotal(otMs),         "var(--purple)"],
          ].map(([label, val, color]) => (
            <div key={label} style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:12, padding:"16px 20px" }}>
              <div style={{ fontSize:11, color:"var(--muted)", fontWeight:600, textTransform:"uppercase", letterSpacing:.8, marginBottom:8 }}>{label}</div>
        ...
        <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:12, padding:16, marginBottom:16 }}>
          ...
          <p style={{ marginTop:8, color:"var(--muted)", fontSize:11 }}>💡 Default shows last 30 days. Adjust dates to see more or less.</p>
        </div>
...
export default function App() {
  ...
  // Real-time listeners for employees + active sessions
  useEffect(() => {
    const unsubEmp = onSnapshot(query(collection(db, "employees"), orderBy("createdAt", "asc")), snap => {
      setEmployees(snap.docs.map(d => ({ id: d.id, ...d.data() })));
      setLoaded(true);
    }, err => { console.error("Employees listener error:", err); setLoaded(true); });

    const unsubAct = onSnapshot(collection(db, "activeSessions"), snap => {
      const sessions = {};
      snap.docs.forEach(d => { sessions[d.id] = d.data(); });
  ...
  const safe = async (fn, errMsg = "Operation failed") => {
    try { await fn(); }
    catch (e) { console.error(e); showToast(errMsg, "error"); }
  };

  // ─── Employee management ───────────────────────────────────────────────
  const handleAddEmployee    = (name) => safe(() => addDoc(collection(db, "employees"), { name, createdAt: Date.now() }), "Couldn't add employee");
  const handleRemoveEmployee = (id)   => safe(() => deleteDoc(doc(db, "employees", id)), "Couldn't remove employee");

  // ─── Sessions ──────────────────────────────────────────────────────────
  const handleClockIn = async (name) => {
    // Check if already has regular entry today (read once, not via listener)
    const today = todayLA();
    const q = query(collection(db, "entries"), where("employee", "==", name), where("date", "==", today), where("type", "==", "regular"));
    return safe(async () => {
      const { getDocs } = await import("firebase/firestore");
      const existing = await getDocs(q);
      if (!existing.empty) { showToast("You've already done your regular shift today", "error"); return; }
      const newSession = { type: "regular", clockInTs: Date.now(), status: "working", pausedAt: null, totalPausedMs: 0 };
      await setDoc(doc(db, "activeSessions", name), newSession);
  ...
  const handleStartOT = async (name) => {
    ...
    return safe(async () => {
      ...
      if (snap.empty) { showToast("No regular entry found for today", "error"); return; }
      const reg = snap.docs[0];
      if (!reg.data().otAuthorized) { showToast("Overtime not authorized", "error"); return; }
      const newSession = { type: "overtime", clockInTs: Date.now(), status: "working", pausedAt: null, totalPausedMs: 0, parentEntryId: reg.id };
      await setDoc(doc(db, "activeSessions", name), newSession);
  ...
  const handleResume = (name) => safe(async () => {
    ...
    if (s.type === "regular" && workedMs(s) >= DAILY_LIMIT_MS) {
      showToast("8h limit reached. You must clock out.", "error");
      return;
    }
    const breakLength = Date.now() - s.pausedAt;
    await updateDoc(doc(db, "activeSessions", name), {
      status: "working",
      pausedAt: null,
      totalPausedMs: (s.totalPausedMs || 0) + breakLength
    });
  ...
  const handleClockOut = (name) => safe(async () => {
    ...
    let workedMillis = outTs - s.clockInTs - totalPaused;
    if (s.type === "regular" && workedMillis > DAILY_LIMIT_MS) workedMillis = DAILY_LIMIT_MS;

    const baseEntry = {
      type: s.type,
      employee: name,
      date: fmtDate(s.clockInTs),
      clockIn: s.clockInTs,
      clockOut: outTs,
      breakMs: totalPaused,
      duration: msToHMS(workedMillis),
    };
    const entry = s.type === "regular"
      ? { ...baseEntry, otAuthorized: false }
      : { ...baseEntry, parentEntryId: s.parentEntryId || null };
  ...
  // ─── Entry management ───────────────────────────────────────────────────
  const handleSaveEntry     = (u)  => safe(() => updateDoc(doc(db, "entries", u.id), { date: u.date, clockIn: u.clockIn, clockOut: u.clockOut, duration: u.duration }), "Update failed");
  const handleDeleteEntry   = (id) => safe(() => deleteDoc(doc(db, "entries", id)), "Delete failed");
  const handleToggleEntryOT = (id) => safe(async () => {
    const { getDoc } = await import("firebase/firestore");
    const ref = doc(db, "entries", id);
    const snap = await getDoc(ref);
    if (!snap.exists()) return;
    await updateDoc(ref, { otAuthorized: !snap.data().otAuthorized });
  }, "Toggle failed");
  ...
  return (
    <>
      ...
      {view === "admin"       && <AdminDashboard employees={employees} activeSessions={activeSessions} onSaveEntry={handleSaveEntry} onDeleteEntry={handleDeleteEntry} onToggleEntryOT={handleToggleEntryOT} onAddEmployee={handleAddEmployee} onRemoveEmployee={handleRemoveEmployee} onBack={() => setView("home")} />}
    </>
  );
}
