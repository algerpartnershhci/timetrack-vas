...
function sumMs(list) {
  return list.reduce((acc, e) => acc + (e.clockOut - e.clockIn - (e.breakMs || 0)), 0);
}
...
const CSS = `
  ...
  *,*::before,*::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    --navy: #0f1117; --card: #1e2333; --border: #2a2f45;
    --accent: #4f8ef7; --green: #3ecf8e; --red: #f76b6b;
    --amber: #f5a623; --purple: #a78bfa; --muted: #7a82a0; --text: #e8eaf0;
  }
...
function Toast({ msg, type }) {
  ...
  const bg = colors[type] || "var(--green)";
  return (
  ...
  return (
    <div style={{ textAlign:"center", padding:"28px 0 16px" }}>
      ...
      <div style={{ color:"var(--muted)", fontSize:13, marginTop:6 }}>
        {t.toLocaleDateString("en-US", { timeZone:TZ, weekday:"long", year:"numeric", month:"long", day:"numeric" })}
      </div>
    </div>
  );
}
...
function Btn({ onClick, children, variant = "primary", disabled, fullWidth, size = "md" }) {
  ...
  return (
    <button
      ...
      style={{ border:"none", cursor:disabled?"not-allowed":"pointer", borderRadius:10, fontFamily:"'DM Sans',sans-serif", fontWeight:600, transition:"all .18s", display:"inline-flex", alignItems:"center", justifyContent:"center", gap:8, width:fullWidth?"100%":"auto", opacity:disabled?.45:1, ...sizes[size], ...variants[variant] }}
    >
      {children}
    </button>
  );
...
function ProgressBar({ ms }) {
  const pct = Math.min(100, (ms / DAILY_LIMIT_MS) * 100);
  ...
  return (
    <div style={{ marginTop:14 }}>
      <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"var(--muted)", fontWeight:600, marginBottom:6, textTransform:"uppercase", letterSpacing:.7 }}>
        <span>Daily Progress</span><span>{msToHMS(ms)} / 08:00:00</span>
      </div>
    ...
    <div style={{ maxWidth:360, margin:"0 auto", padding:20 }}>
      ...
      <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:16, padding:28, textAlign:"center" }}>
        ...
        <div style={{ display:"flex", gap:16, justifyContent:"center", marginBottom:16, animation:shake?"shake .4s ease":"none" }}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{ width:18, height:18, borderRadius:"50%", background:i < pin.length ? "var(--accent)" : "transparent", border:`2px solid ${i < pin.length ? "var(--accent)" : "var(--border)"}`, transition:"all .15s" }} />
          ))}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, maxWidth:240, margin:"0 auto" }}>
          {[1,2,3,4,5,6,7,8,9,null,0,"del"].map((d, i) => {
            ...
            const isDel = d === "del";
            return (
              <button key={i} onClick={() => isDel ? pressDel() : pressDigit(String(d))}
                style={{ background:isDel?"var(--border)":"var(--navy)", border:"1.5px solid var(--border)", borderRadius:12, padding:"16px 0", color:"var(--text)", fontSize:isDel?14:22, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans'", display:"flex", alignItems:"center", justifyContent:"center" }}>
...
function HomeScreen({ onEmployee, onAdmin }) {
  return (
    <div style={{ minHeight:"100vh", background:"var(--navy)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ marginBottom:12, background:"var(--accent)", borderRadius:16, padding:14, display:"inline-flex" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14, width:"100%", maxWidth:340 }}>
        <button onClick={onEmployee}
          style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:14, color:"var(--text)", padding:"20px 24px", cursor:"pointer", textAlign:"left", fontFamily:"'DM Sans'" }}
          onMouseEnter={e => e.currentTarget.style.borderColor = "var(--accent)"}
          onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:6 }}>
            <User size={20} color="var(--accent)" />
            <span style={{ fontWeight:700, fontSize:16 }}>I'm an Employee</span>
          </div>
...
<h2>Regular Shifts</h2>${regular.length > 0 ? `<table><thead><tr><th>Date</th><th>Clock In</th><th>Clock Out</th><th>Break</th><th>Total</th></tr></thead><tbody>${regRows}</tbody></table><div class="sum"><div>Entries: <span>${regular.length}</span></div><div>Total Hours: <span>${fmtTotal(sumMs(regular))}</span></div></div>` : '<p style="color:#999;font-style:italic;padding:12px 0">No regular shifts in this period.</p>'}
<h2 class="ot">Overtime</h2>${ot.length > 0 ? `<table><thead><tr><th class="ot">Date</th><th class="ot">Clock In</th><th class="ot">Clock Out</th><th class="ot">Break</th><th class="ot">Total</th></tr></thead><tbody>${otRows}</tbody></table><div class="sum"><div>OT Entries: <span>${ot.length}</span></div><div>OT Hours: <span style="color:#7c3aed">${fmtTotal(sumMs(ot))}</span></div></div>` : '<p style="color:#999;font-style:italic;padding:12px 0">No overtime in this period.</p>'}
<div class="footer">All times in PST/PDT (Los Angeles, California) &bull; Combined total: ${fmtTotal(sumMs(regular)+sumMs(ot))}</div></body></html>`;
      ...
      {loading ? (
        ...
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:14 }}>
            {[["Regular",regular.length,"var(--accent)"],["Reg Hrs",fmtTotal(sumMs(regular)),"var(--green)"],["OT",ot.length,"var(--purple)"],["OT Hrs",fmtTotal(sumMs(ot)),"var(--purple)"]].map(([l,v,c]) => (
          ...
          {ot.length > 0 && (
            <div>
              <div style={{ fontSize:11, fontWeight:700, color:"var(--purple)", textTransform:"uppercase", letterSpacing:.8, marginBottom:8, display:"flex", alignItems:"center", gap:6 }}>
...
function EmployeeScreen({ employees, activeSessions, onClockIn, onStartOT, onPause, onResume, onClockOut, onBack }) {
  ...
  // Subscribe to today's entries once PIN verified
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (s && s.type === "regular" && s.status === "working" && workedMs(s) >= DAILY_LIMIT_MS) {
      onPause(selected);
    }
          ...
          {employees.length === 0 ? (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {employees.map(emp => {
                ...
                return (
                  <button key={emp.id} onClick={() => selectEmployee(emp.name)}
                    style={{ background:"var(--navy)", border:"1.5px solid var(--border)", borderRadius:10, padding:"12px 16px", cursor:"pointer", textAlign:"left", color:"var(--text)", fontFamily:"'DM Sans'", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
  ...
  // PIN not yet verified
  if (!pinVerified) {
    if (!selEmp?.pin) {
      return (
        <div style={{ minHeight:"100vh", background:"var(--navy)", padding:20, maxWidth:480, margin:"0 auto" }}>
          <button onClick={switchBack} style={{ background:"none", border:"none", color:"var(--muted)", cursor:"pointer", display:"flex", alignItems:"center", gap:6, fontSize:14, marginBottom:20, fontFamily:"'DM Sans'" }}>
            <ArrowLeft size={16} /> Back
          </button>
    ...
    return (
      <div style={{ minHeight:"100vh", background:"var(--navy)" }}>
        <PinEntry employee={selEmp} onSuccess={() => setPinVerified(true)} onBack={switchBack} />
      </div>
    );
  }

  // PIN verified — show tabs
  return (
    <div style={{ minHeight:"100vh", background:"var(--navy)", padding:20, maxWidth:480, margin:"0 auto" }}>
      <LiveClock />
      ...
      <div style={{ display:"flex", gap:8, marginBottom:14 }}>
        {[["today","Today's Shift"],["records","My Records"]].map(([v, label]) => (
          <button key={v} onClick={() => setEmpView(v)}
            style={{ flex:1, background:empView===v?"var(--accent)":"var(--card)", border:empView===v?"1.5px solid var(--accent)":"1.5px solid var(--border)", borderRadius:10, padding:"10px 0", color:empView===v?"#fff":"var(--muted)", fontWeight:600, fontSize:13, cursor:"pointer", fontFamily:"'DM Sans'", transition:"all .18s" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {empView === "records" && (
        <MyRecords employeeName={selected} onSwitch={switchBack} />
      )}
      ...
      {empView === "today" && (
        <div style={{ background:"var(--card)", border:session?.type==="overtime"?"1.5px solid var(--purple)":"1.5px solid var(--border)", borderRadius:14, padding:20 }}>
          ...
          {session?.type === "regular" && (
            <RegularSessionView session={session} onPause={() => onPause(selected)} onResume={() => onResume(selected)} onClockOut={() => onClockOut(selected)} />
          )}

          {session?.type === "overtime" && (
            <OvertimeSessionView session={session} onPause={() => onPause(selected)} onResume={() => onResume(selected)} onClockOut={() => onClockOut(selected)} />
          )}

          {!session && !todayRegular && (
            <div style={{ marginTop:14 }}>
              <p style={{ color:"var(--muted)", fontSize:13, marginBottom:14 }}>Ready to start your shift?</p>
              <Btn variant="green" fullWidth onClick={() => onClockIn(selected)}>
          ...
          {!session && todayRegular && (
            <div>
              <div style={{ background:"rgba(62,207,142,.08)", border:"1px solid rgba(62,207,142,.25)", borderRadius:10, padding:16, marginTop:14 }}>
                ...
                {todayRegular.breakMs > 0 && (
                  <div style={{ marginTop:10, fontSize:12, color:"var(--muted)" }}>Break: <span style={{ color:"var(--amber)", fontFamily:"'DM Mono'" }}>{msToHMS(todayRegular.breakMs)}</span></div>
                )}
              </div>

              {todayOTs.length > 0 && (
                <div style={{ background:"rgba(167,139,250,.08)", border:"1px solid rgba(167,139,250,.3)", borderRadius:10, padding:16, marginTop:12 }}>
                  <div style={{ color:"var(--purple)", fontWeight:700, fontSize:13, marginBottom:10, display:"flex", alignItems:"center", gap:6 }}>
                    <Zap size={14} /> Overtime today ({todayOTs.length})
                  </div>
                  {todayOTs.map(ot => (
              ...
              {otAuth && (
                <div style={{ marginTop:14 }}>
                  <Btn variant="purple" fullWidth onClick={() => onStartOT(selected)}>
                    <Zap size={18} /> Start Overtime
                  </Btn>
                  <p style={{ color:"var(--muted)", fontSize:12, textAlign:"center", marginTop:10 }}>Overtime authorized by admin</p>
                </div>
              )}

              {!otAuth && todayOTs.length === 0 && (
  ...
  return (
    <div>
      ...
      <div style={{ background:"var(--navy)", borderRadius:10, padding:18, marginTop:14, textAlign:"center" }}>
        <div style={{ fontSize:11, color:"var(--muted)", fontWeight:600, textTransform:"uppercase", letterSpacing:.7, marginBottom:6 }}>
          {session.status === "working" ? "Time Worked" : "On Break"}
        </div>
        ...
        <div style={{ fontSize:12, color:"var(--muted)", marginTop:6 }}>
          ...
          {(session.totalPausedMs > 0 || (session.status === "paused" && session.pausedAt)) && (
            <span> • Break: {msToHMS((session.totalPausedMs||0) + (session.status==="paused"&&session.pausedAt ? Date.now()-session.pausedAt : 0))}</span>
          )}
        </div>
        <ProgressBar ms={w} />
      </div>
      <div style={{ display:"flex", gap:8, marginTop:14 }}>
        {session.status === "working" && !atLimit && (
          <Btn variant="amber" fullWidth onClick={onPause}><Pause size={18} /> Take Break</Btn>
        )}
        {session.status === "paused" && !atLimit && (
          <Btn variant="green" fullWidth onClick={onResume}><Play size={18} /> Resume</Btn>
        )}
...
function OvertimeSessionView({ session, onPause, onResume, onClockOut }) {
  return (
    <div>
      ...
      <div style={{ background:"var(--navy)", borderRadius:10, padding:18, marginTop:14, textAlign:"center" }}>
        <div style={{ fontSize:11, color:"var(--muted)", fontWeight:600, textTransform:"uppercase", letterSpacing:.7, marginBottom:6 }}>
          {session.status === "working" ? "Overtime Worked" : "On Break"}
        </div>
...
function AdminLogin({ onAuth, onBack }) {
  ...
  const attempt = () => {
    if (pw.trim() === ADMIN_PASSWORD) onAuth();
    else { setErr(true); setTimeout(() => setErr(false), 2200); }
  };

  return (
    <div style={{ minHeight:"100vh", background:"var(--navy)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      <button onClick={onBack} style={{ position:"absolute", top:20, left:20, background:"none", border:"none", color:"var(--muted)", cursor:"pointer", display:"flex", alignItems:"center", gap:6, fontSize:14, fontFamily:"'DM Sans'" }}>
        <ArrowLeft size={16} /> Back
      </button>
      <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:16, padding:32, width:"100%", maxWidth:360 }}>
        <div style={{ textAlign:"center", marginBottom:24 }}>
          <div style={{ background:"rgba(62,207,142,.15)", borderRadius:14, padding:14, display:"inline-flex", marginBottom:12 }}>
...
function EditModal({ entry, onSave, onClose }) {
  ...
  const save = () => {
    const inTs  = laToUTC(dateStr, clockInStr);
    const outTs = laToUTC(dateStr, clockOutStr);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:20 }}>
      <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:16, padding:28, width:"100%", maxWidth:400 }}>
        <h3 style={{ fontWeight:700, fontSize:17, marginBottom:4 }}>
          Edit {entry.type==="overtime"?"Overtime":"Time"} Entry
          {entry.type==="overtime" && <Zap size={16} color="var(--purple)" style={{ marginLeft:8, verticalAlign:"middle" }} />}
        </h3>
...
function EmployeeManagement({ employees, onAdd, onRemove, onSetPin }) {
  ...
  const add = () => {
    ...
    onAdd(name, newPin);
    setNewName(""); setNewPin(""); setPinErr("");
  };

  const savePin = (id) => {
    if (!validPin(pinInput)) { setPinErr("PIN must be exactly 4 digits"); return; }
    onSetPin(id, pinInput);
    setEditPinId(null); setPinInput(""); setPinErr("");
  };
    ...
    <div>
      ...
      <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:12, padding:16, marginBottom:16 }}>
        <div style={{ fontSize:12, fontWeight:700, color:"var(--muted)", textTransform:"uppercase", letterSpacing:.8, marginBottom:12, display:"flex", alignItems:"center", gap:8 }}>
          <Users size={14} /> Manage Employees ({employees.length})
        </div>
        ...
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex", gap:8, marginBottom:6 }}>
            <input type="text" value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key==="Enter"&&add()} placeholder="Full name"
              style={{ flex:3, background:"var(--navy)", border:`1.5px solid ${pinErr?"var(--red)":"var(--border)"}`, borderRadius:8, color:"var(--text)", padding:"10px 14px", fontSize:14, fontFamily:"'DM Sans'", outline:"none" }} />
            <input type="text" value={newPin} onChange={e => { setNewPin(e.target.value.replace(/\D/g,"")); setPinErr(""); }} onKeyDown={e => e.key==="Enter"&&add()} placeholder="PIN" maxLength={4}
              style={{ width:80, background:"var(--navy)", border:`1.5px solid ${pinErr?"var(--red)":"var(--border)"}`, borderRadius:8, color:"var(--text)", padding:"10px 14px", fontSize:14, fontFamily:"'DM Mono'", outline:"none", textAlign:"center", letterSpacing:4 }} />
              ...
              <div key={emp.id} style={{ background:"var(--navy)", borderRadius:8, padding:"10px 14px" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize:11, marginTop:2, display:"flex", alignItems:"center", gap:6, color:"var(--muted)" }}>
                      <Lock size={10} />
                      {emp.pin
                        ? <span style={{ fontFamily:"'DM Mono'", color:"var(--accent)", fontWeight:600, letterSpacing:3 }}>PIN: {emp.pin}</span>
                        : <span style={{ color:"var(--amber)" }}>⚠ No PIN set</span>}
                    </div>
                  </div>
                  <div style={{ display:"flex", gap:6 }}>
                    <button onClick={() => { setEditPinId(emp.id); setPinInput(""); setPinErr(""); }}
                    ...
                    <button onClick={() => setRemoveId(emp.id)}
                      style={{ background:"rgba(247,107,107,.15)", border:"none", borderRadius:6, padding:"5px 8px", cursor:"pointer", color:"var(--red)", display:"flex", alignItems:"center" }}>
                      <X size={13} />
                    </button>
                  </div>
                </div>
                ...
                {editPinId === emp.id && (
                  <div style={{ marginTop:10 }}>
                    <div style={{ display:"flex", gap:8, marginBottom:6 }}>
                      <input type="text" value={pinInput} onChange={e => { setPinInput(e.target.value.replace(/\D/g,"")); setPinErr(""); }} placeholder="New PIN" maxLength={4} autoFocus
                        style={{ flex:1, background:"var(--card)", border:`1.5px solid ${pinErr?"var(--red)":"var(--border)"}`, borderRadius:8, color:"var(--text)", padding:"8px 12px", fontSize:16, fontFamily:"'DM Mono'", outline:"none", textAlign:"center", letterSpacing:6 }} />
...
function ActiveSessions({ activeSessions }) {
  ...
  return (
    <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:12, padding:16, marginBottom:16 }}>
      <div style={{ fontSize:12, fontWeight:700, color:"var(--muted)", textTransform:"uppercase", letterSpacing:.8, marginBottom:12, display:"flex", alignItems:"center", gap:8 }}>
        <span style={{ width:8, height:8, borderRadius:"50%", background:"var(--green)", boxShadow:"0 0 0 3px rgba(62,207,142,.3)" }} />
        Active Right Now ({list.length})
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {list.map(([name, s]) => {
          const w = workedMs(s);
          const isOT = s.type === "overtime";
          const atLimit = !isOT && w >= DAILY_LIMIT_MS;
          return (
            <div key={name} style={{ background:"var(--navy)", border:isOT?"1px solid rgba(167,139,250,.4)":"1px solid var(--border)", borderRadius:10, padding:14 }}>
              ...
              <div style={{ fontSize:12, color:"var(--muted)" }}>
                In: <span style={{ fontFamily:"'DM Mono'", color:"var(--text)" }}>{fmtTime(s.clockInTs)}</span> • Worked: <span style={{ fontFamily:"'DM Mono'", color:isOT?"var(--purple)":atLimit?"var(--red)":"var(--green)" }}>{msToHMS(w)}</span>
              </div>
            </div>
          );
        })}
...
function EntriesTable({ entries, type, onEdit, onDelete, onToggleOT }) {
  ...
  const headers = isOT
    ? ["Employee","Date","Clock In","Clock Out","Break","Total","Actions"]
    : ["Employee","Date","Clock In","Clock Out","Break","Total","Authorize OT","Actions"];
            ...
            <tr key={e.id} style={{ borderBottom:i<entries.length-1?"1px solid var(--border)":"none" }}>
              ...
              {!isOT && (
                <td style={{ padding:"12px 14px" }}>
                  <button onClick={() => onToggleOT(e.id)}
      ...
      <div style={{ maxWidth:1100, margin:"0 auto" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <button onClick={onBack} style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:8, color:"var(--muted)", cursor:"pointer", display:"flex", padding:8 }}>
              <ArrowLeft size={18} />
            </button>
            <div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <a href="https://github.com/algerpartnershhci/timetrack-vas/blob/main/src/App.jsx" target="_blank" rel="noopener noreferrer"
              style={{ fontSize:11, color:"var(--muted)", textDecoration:"none" }}>⚙️ Change password</a>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
          {[["Regular Entries",regular.length,"var(--accent)"],["Regular Hours",fmtTotal(sumMs(regular)),"var(--green)"],["OT Entries",ot.length,"var(--purple)"],["OT Hours",fmtTotal(sumMs(ot)),"var(--purple)"]].map(([l,v,c]) => (
          ...
          <div>
            <div style={{ background:"var(--card)", border:"1.5px solid var(--border)", borderRadius:12, overflow:"hidden", marginBottom:20 }}>
              <div style={{ padding:"14px 20px", borderBottom:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <span style={{ color:"var(--muted)", fontSize:13 }}>{regular.length} record{regular.length!==1?"s":""}</span>
              </div>
              <EntriesTable entries={regular} type="regular" onEdit={setEditEntry} onDelete={setDelConfirm} onToggleOT={id=>{onToggleEntryOT(id);showToast("OT authorization updated");}} />
            </div>
...
export default function App() {
  ...
  useEffect(() => {
    const unsubEmp = onSnapshot(
      query(collection(db,"employees"), orderBy("createdAt","asc")),
      snap => { setEmployees(snap.docs.map(d=>({id:d.id,...d.data()}))); setLoaded(true); },
      () => setLoaded(true)
    );
    const unsubAct = onSnapshot(
      collection(db,"activeSessions"),
      snap => { const s={}; snap.docs.forEach(d=>{s[d.id]=d.data();}); setActiveSessions(s); }
    );
  ...
  const handleClockOut = (name) => safe(async () => {
    ...
    const entry = {
      type: s.type, employee: name, date: fmtDate(s.clockInTs),
      clockIn: s.clockInTs, clockOut: outTs, breakMs: totalPaused, duration: msToHMS(worked),
      ...(s.type==="regular" ? {otAuthorized:false} : {parentEntryId:s.parentEntryId||null})
    };
